...
import { roundNearestFive, getRaceBoosts } from "./stats.js";
...
export function buildCardEmbed(card, ownedEntry, viewer) {
  ...
  // Add race at the top
  if (card.race) {
    statsParts.push(`**Race:** ${card.race}`);
  }
  ...
  if (card.specialAttack) {
    ...
    let specialText = `**Special:** ${card.specialAttack.name} (${smin}-${smax} damage)`;
    if (card.specialAttack.specialCondition) {
      specialText += ` [${card.specialAttack.specialCondition}]`;
    }
    statsParts.push(specialText);
  }
...
export function buildUserCardEmbed(card, ownedEntry, viewer, equippedWeapon = null, teamBanner = null) {
  ...
  // Calculate race boosts and display as percentages
  const raceBoost = getRaceBoosts(card.race);
  const raceAtkPercent = (raceBoost.power || 0) * 100;
  const raceHpPercent = (raceBoost.health || 0) * 100;
  
  // If user has a generated boost assigned to this owned entry, show it with race boosts
  if (ownedEntry && ownedEntry.boost) {
    const b = ownedEntry.boost;
    const bparts = [];
    ...
    if (raceAtkPercent > 0) bparts.push(`Race: ATK +${raceAtkPercent}%`);
    if (raceHpPercent > 0) bparts.push(`Race: HP +${raceHpPercent}%`);
    if (bparts.length) statsParts.push(`**Boosts:** ${bparts.join(' • ')}`);
  } else if (raceAtkPercent > 0 || raceHpPercent > 0) {
    // If only race boosts exist, always show them
    const bparts = [];
    if (raceAtkPercent > 0) bparts.push(`Race: ATK +${raceAtkPercent}%`);
    if (raceHpPercent > 0) bparts.push(`Race: HP +${raceHpPercent}%`);
    if (bparts.length) statsParts.push(`**Boosts:** ${bparts.join(' • ')}`);
